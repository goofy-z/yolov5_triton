...
def postprocess(prediction, origin_w, origin_h, input_shape, conf_th=0.5, nms_threshold=0.5, letter_box=False):
    # Here we use the first row of output in that batch_size = 1
    prediction = prediction[0]

    xc = prediction[...,4]>conf_th

    output=prediction[xc]

    output[:,5:]*=output[:,4:5]

    # box = xywh2xyxyv5(output[:,:4])
    box = output[:,:4]


    conf=output[:,5:].max(1,keepdims=True)
    j=output[:,5:].argmax(1).reshape(-1,1)
    pred=np.concatenate((box,conf,j.astype(np.float)),axis=1)[conf.reshape(-1)>conf_th]

    # pred: detections, (x1, y1, x2, y2, conf, cls_id)
    # Do nms
